...
function encodeToRot13(s) {
  const enc = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.split('');
  const bias = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return s
    .split('')
    .map((el) => (enc.includes(el) ? bias[enc.indexOf(el)] : el))
    .join('');
}
